				...
				<div>
					...
					>
						...
						</div>
						{
							...
							.slice()
							.reverse()
							// url(&quot;/api/static/cartoon%2FThe%20Tutorial%20Is%20Too%20Hard%2Fbackground.png&quot;);
							.map((value, key) => {
								return (
								<button key={key}>
									<div className="overflow-hidden w-full h-full rounded-xl" style={{backgroundImage: `url(/api/static/${encodeURIComponent(value.image_background)})`}}>
										<Link to={`/cartoon/${value.name}`}>
											<div className=" overflow-hidden w-full h-full rounded-xl" style={{ backgroundImage: `url(/api/static/${encodeURIComponent(value.image_background)})` }}>
												<img src={`/api/static/${value.image_main}`} className="object-cover scale-100  hover:scale-105 duration-300" />
											</div>
										</Link>
									</div>
								</button>
